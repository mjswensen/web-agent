...
{#if app.hasQueuedMessages}
	<section
		class="border-t border-slate-200 bg-amber-50 px-4 py-2 sm:px-6 dark:border-amber-900 dark:bg-amber-950"
		aria-label="Queued messages"
	>
		<div class="mx-auto w-full max-w-4xl">
			<button
				type="button"
				class="min-h-11 rounded-md px-2 text-xs font-semibold text-amber-900 underline-offset-2 hover:underline focus:ring-2 focus:ring-amber-500 focus:outline-none dark:text-amber-200"
			...
			{#if app.layout.queueOpen}
				<div class="grid gap-3 pb-2 text-xs text-amber-950 sm:grid-cols-2 dark:text-amber-100">
					<section>
